...
function BackgroundCheckNotice() {
  return (
    <p className="hint-sm" style={{ background: "var(--field-bg)", border: "1px solid var(--border)", borderRadius: "0.5rem", padding: "0.65rem 0.8rem", marginBottom: "0.9rem" }}>
      Background checks are run by our third-party partner, <strong>Checkr</strong>, and paid for directly by you —
      Roost Assured never collects or handles any part of that fee.
    </p>
  )
}
...
export default function Account() {
  ...
  const refreshPaymentMethod = () => {
    api.get("/stripe_payment_method").then((r) => setHasPaymentMethod(r.has_payment_method))
  }
  ...
  const handleSitterSubmit = async (e) => {
    ...
    try {
      let result
      if (isSitter) {
        result = await api.patch("/sitter_profile", buildSitterProfileFormData())
      } else if (application) {
        result = await api.patch("/sitter_application", buildApplicationFormData())
      } else {
        result = await api.post("/sitter_application", buildApplicationFormData())
      }
      setForm(result.user)
      setSitterForm(result.user.sitter || result.user.sitter_application)
      setUser(result.user)
      setNotice(isSitter ? "Sitter profile updated." : "Application submitted — we'll email you once it's reviewed.")
    } catch (err) {
      setErrors(err.data?.errors || ["Something went wrong."])
    } finally {
            ...
            <div>
                ...
                <form onSubmit={handleSitterSubmit}>
                  ...
                  <div className="field" style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    ...
                    <label htmlFor="background_check_consent" style={{ marginBottom: 0 }}>
                      ...
                      <Tooltip text="Run by Checkr, a third-party background check provider. You pay Checkr directly — Roost Assured never touches that payment." />
                    </label>
                  </div>
              ...
              {!isSitter && (
                <form onSubmit={handleSitterSubmit}>
                  ...
                  <button type="submit" className="btn btn-primary" disabled={sitterSubmitting} style={{ width: "100%" }}>
                    {sitterSubmitting ? "Saving…" : application ? "Update application" : "Submit application"}
                  </button>
                </form>
              )}
